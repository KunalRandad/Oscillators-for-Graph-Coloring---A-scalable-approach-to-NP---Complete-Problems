...
def graph_painter_exp_attraction_visual(adj, num_iters = 3, phi_init = None, central_term_only = False, 
                                        pause_duration = 0.01, reset_prob = 0.01,
                                        step_size_slider_init = -5, sigma_exp_slider_init = 0,
    ...
    sigma_exp_slider = widgets.Slider(ax_sigma_exp, 'Sigma Exp', valmin=-7, valmax=2,
                                       valinit=sigma_exp_slider_init, valstep=0.01, color=axcolor)
    ...
    for i in range(num_iters):
        ...
        # With probability reset_prob, reset the current phi for each node independently to 0
        for j in range(n):
...
if __name__ == "__main__":
    ...
    num_colors, best_coloring, phi_history = graph_painter_exp_attraction_visual(adj, num_iters,
                                            central_term_only=False, pause_duration = 0.001,
                                            reset_prob = 0.000,
                                            step_size_slider_init=-7,
                                            sigma_exp_slider_init=0, 
                                            sigma_noise_slider_init=-7,
                                            sigma_attraction_slider_init=0,
                                            attraction_step_size_slider_init=-7)
    # Save the phi_history to a file
    phi_history = np.array(phi_history)
    np.save('phi_history_exponential_'+graph_name+'.npy', phi_history)
