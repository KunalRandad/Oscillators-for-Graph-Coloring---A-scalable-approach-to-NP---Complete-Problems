...
def graph_painter_exp_attraction_visual(adj, num_iters = 3, phi_init = None, central_term_only = False, 
                                        pause_duration = 0.01,
                                        step_size_slider_init = -5, sigma_gaussian_slider_init = 0,
                                        sigma_noise_slider_init = -5, sigma_attraction_slider_init = 0,
                                        attraction_step_size_slider_init = -5):
    n = adj.shape[0]  
    num_nodes = n                          
    ...
    step_size_slider = widgets.Slider(ax_step_size, 'Step Size', valmin=-7, valmax=2,
                                       valinit=step_size_slider_init, valstep=0.01, color=axcolor)
    
    ax_sigma_gaussian = plt.axes([0.1, 0.93, 0.15, 0.04], facecolor=axcolor)
    sigma_gaussian_slider = widgets.Slider(ax_sigma_gaussian, 'Sigma Gaussian', valmin=-7, valmax=2,
                                       valinit=sigma_gaussian_slider_init, valstep=0.01, color=axcolor)
    ...
    sigma_noise_slider = widgets.Slider(ax_sigma_noise, 'Sigma Noise', valmin=-7, valmax=2,
                                         valinit=sigma_noise_slider_init, valstep=0.01, color=axcolor)
    ...
    sigma_attraction_slider = widgets.Slider(ax_sigma_attraction, 'Sigma Attraction', valmin=-7, valmax=2,
                                              valinit=sigma_attraction_slider_init, valstep=0.01, color=axcolor)
    ...
    attraction_step_size_slider = widgets.Slider(ax_attraction_step_size, 'Attraction Step Size', valmin=-7, valmax=2,
                                                  valinit=attraction_step_size_slider_init, valstep=0.01, color=axcolor)
    ...
    ax_sigma_gaussian_text_box = plt.axes([0.25, 0.935, 0.05, 0.03], facecolor=axcolor)
    sigma_gaussian_text_box = widgets.TextBox(ax_sigma_gaussian_text_box, label = '', initial=str(10**sigma_gaussian_slider.val))
    ...
    # This indicates the frequency of color of the corresponding node
    color_freq = np.ones((1,n))
    ...
    for i in range(num_iters):
        ...
        sigma_gaussian = 10**sigma_gaussian_slider.val
        ...
        sigma_gaussian_text_box.set_val(f'{sigma_gaussian:.2e}')
        ...
        term1 = (-1 / sigma_gaussian**2) * x * np.exp(-x**2 / (2 * sigma_gaussian**2))
        term1[x == 0] = 0
        np.fill_diagonal(term1, 0) # to avoid self-derivative
        term2 = (-1 / sigma_gaussian**2) * (x - 2 * np.pi) * np.exp(- (x - 2 * np.pi)**2 / (2 * sigma_gaussian**2))                                         
        np.fill_diagonal(term2, 0)
        term3 = (-1 / sigma_gaussian**2) * (x + 2 * np.pi) * np.exp(- (x + 2 * np.pi)**2 / (2 * sigma_gaussian**2))
        ...
        # Scale adj by the color frequency
        # adj = np.asarray(adj, dtype=np.float64)
        # print("adj shape: ", adj.shape)
        # print("curr_phi shape: ", curr_phi.shape)
        # print("loss_derivative_clip shape: ", loss_derivative_clip.shape)
        # For adj_scaled, divide each column by the color frequency of the corresponding node
        # Perform element-wise division for each row of adj by the corresponding color frequency
        # adj_scaled1 = np.ones_like(adj, dtype=np.float64)
        # for j in range(n):
        #     # Perform element-wise division
        #     adj_scaled1 = 
        # for j in range(n):
        #     for k in range(n):
        #         adj_scaled1[j, k] = adj[j, k] / color_freq[0, k]
        # adj_scaled2 = np.ones_like(adj, dtype=np.float64)
        # adj_scaled2 = adj/ color_freq
        adj_scaled = adj / color_freq
        curr_phi = curr_phi - step_size* np.sum(adj_scaled*loss_derivative_clip, axis=1) + attraction_step_size * np.sum(attraction_coeff*attraction_derivative_clip, axis=1)
        ...
        if num_chr < min_colors:
            ...
            color_adj_graph = get_color_adj_graph(color_indice_array, adj, num_chr)
            density_colored = density(color_adj_graph)
            print(f"Iteration {i}: Found new best coloring with {num_chr} colors, density: {density_colored:.4f}")
...
if __name__ == "__main__":
    ...
    # graph_name = 'color_adj_graph_temp'
    graph_name = 'myciel6'
    ...
    num_colors, best_coloring, phi_history = graph_painter_exp_attraction_visual(adj, num_iters,
                                            ...
                                            step_size_slider_init=-7,
                                            sigma_gaussian_slider_init=0, 
                                            sigma_noise_slider_init=-7,
                                            sigma_attraction_slider_init=0,
                                            attraction_step_size_slider_init=-7)
    color_indice_array, _ = get_color_indice_array(best_coloring, adj)
    color_adj_matrix = get_color_adj_graph(color_indice_array, adj, num_colors)
    color_adj_graph = nx.from_numpy_array(color_adj_matrix)
    # Save color_adj_graph to a file
    # np.save('test_graphs/color_adj_graph_'+graph_name+'.npy', color_adj_graph)
    # make graph to mat
    graph_to_mat(color_adj_graph, 'test_graphs/color_adj_graph_'+'temp'+'.mat')
    # Save color_adj_matrix as csv with only 1s and 0s without decimal
    np.savetxt('test_graphs/color_adj_graph_'+'temp'+'.csv', color_adj_matrix, delimiter=',', fmt='%d')
