...
def mod_one_exchange(G, initial_cut=None, seed=None, weight=None):
    ...
    # while True:
        ...
        #     default=None,
        # )
        # best_node_to_swap: Calculate the best node to swap by calculating the node that gives the highest gain to the cut value
        # Just find the gain of each node and select the one with the highest gain
        best_node_to_swap = None
        best_gain = -1000000
        for node in nodes:
            if node in cut:
                # gain = adjacent edges in cut - adjacent edges not in cut
                gain = sum(1 for neighbor in G.neighbors(node) if neighbor in cut) - sum(1 for neighbor in G.neighbors(node) if neighbor not in cut)
            else:
                gain = sum(1 for neighbor in G.neighbors(node) if neighbor not in cut) - sum(1 for neighbor in G.neighbors(node) if neighbor in cut)
...
def mod_one_exchange_approximation_new(G):
    n = G.number_of_nodes()
    partition_set = []
